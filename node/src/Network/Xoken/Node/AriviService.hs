...
globalHandlerRpcArivi ::
     (AriviEnv.HasService env m) => RPCMessage -> m (Maybe RPCMessage)
globalHandlerRpcArivi msg = throw UnsupportedMethodException

globalHandlerPubSubArivi ::
     (AriviEnv.HasService env m) => String -> PubNotifyMessage -> m Status
globalHandlerPubSubArivi tpc msg = throw UnsupportedMethodException

globalHandlerPubSub ::
     (HasService env m) => ServiceTopic -> PubNotifyMessage -> m Status
